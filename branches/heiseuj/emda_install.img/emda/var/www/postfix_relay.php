...
function doit($input)
{
 ...
 while($line = fgets($fp,2096)) {
  ...
  $_timestamp = mysql_escape_string($parsed->timestamp);
  $_host = mysql_escape_string($parsed->host);
  ...
  if($parsed->process == 'postfix/smtp' && class_exists('postfix_parser')) {
   ...
   $_msg_id = mysql_escape_string($postfix->id);
   ...
   if((preg_match('/Milter: /i',$postfix->raw)) && (preg_match('/(rejected recipient|user unknown)/i',$postfix->entries['reject']))) {
    $_type = mysql_escape_string('unknown_user');
    $_status = mysql_escape_string(get_email($postfix->entries['to']));
   }
   ...
   if(preg_match('/user unknown/i',$postfix->entry)) {
    // Unknown users
    $_type = mysql_escape_string('unknown_user');
    $_status = mysql_escape_string($postfix->raw);
   }
    ...
    $_status = mysql_escape_string($postfix->raw);
		if(preg_match('/Client host rejected: cannot find your hostname/i',$postfix->entry)) {
			$_type = mysql_escape_string('unknown_hostname');
		}
		else
			$_type = mysql_escape_string('NOQUEUE');
		$_status = mysql_escape_string($postfix->raw);
	 }
   // Relay lines
   if(isset($postfix->entries['relay']) && isset($postfix->entries['status'])) {
    $_type = mysql_escape_string('relay');
    $_delay = mysql_escape_string($postfix->entries['delay']);
    $_relay = mysql_escape_string(get_ip($postfix->entries['relay']));
    $_dsn = mysql_escape_string($postfix->entries['dsn']);
    $_status = mysql_escape_string($postfix->entries['status']);
   }
