...
function lazy($title, $val) {
 echo " <tr>\n";
 echo "  <td class=\"heading\" align=\"right\" width=10%>$title</td>\n";
 echo "  <td class=\"detail\" width=80%>$val</td>\n";
 echo " </tr>\n";
}
...
foreach($mime_struct as $key=>$part) {
 ...
 switch($type) {
  ...
  default:
   echo " <tr>\n";
   echo "  <td colspan=2 class=\"detail\">".$part->d_parameters['filename']." <a href=\"viewpart.php?id=".$_GET['id']."&filename=".$_GET['filename']."&part=".$part->mime_id."\">Download</a></td>";
   echo " </tr>\n";
   break;
 }
}
